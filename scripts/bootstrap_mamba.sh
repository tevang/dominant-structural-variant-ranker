...
usage() {
  cat <<'EOF'
Usage: scripts/bootstrap_mamba.sh [options]
...
Options:
  ...
  --with-xtb        mamba install xtb -c conda-forge
  --with-crest      mamba install crest -c conda-forge; print manual instructions if unavailable
...
if ! command -v mamba >/dev/null 2>&1; then
  echo "mamba is not on PATH. Install mamba/micromamba or use scripts/bootstrap_conda.sh." >&2
  exit 1
fi
...
mamba run -n "$ENV_NAME" python -m pip install -e ".[dev]"
...
if [[ "$WITH_AUTO3D" == "1" ]]; then
  run_optional "Auto3D" mamba run -n "$ENV_NAME" python -m pip install Auto3D
fi

if [[ "$WITH_PYSCF" == "1" ]]; then
  run_optional "PySCF" mamba run -n "$ENV_NAME" python -m pip install pyscf
fi

if [[ "$WITH_PSI4" == "1" ]]; then
  run_optional "Psi4 from conda-forge" mamba install -n "$ENV_NAME" -c conda-forge -y psi4
fi

if [[ "$WITH_XTB" == "1" ]]; then
  run_optional "xTB from conda-forge" mamba install -n "$ENV_NAME" -c conda-forge -y xtb
fi

if [[ "$WITH_CREST" == "1" ]]; then
  if ! run_optional "CREST from conda-forge" mamba install -n "$ENV_NAME" -c conda-forge -y crest; then
    :
  fi
  if ! mamba run -n "$ENV_NAME" bash -lc 'command -v crest >/dev/null 2>&1'; then
    cat >&2 <<'EOF'
...
mamba run -n "$ENV_NAME" dsvr doctor
